...
tipo mergeOrdenado(tipo tA, tipo tB){ // Esta funcion se encarga de unir en un mismo tipo ordenado, a 2 tipos que ya estan ordenados.
...
template<typename tipo>
void mergeSort(tipo &tA){ // Esta es una implementacion de mergesort. No es in-place. Ordena ascendente. Se pasa tipo por referencia.
    if(tA.size()<2){ // Caso base si tengo 1 elem o 0 elem, ya esta ordenado. No debo cambiar nada.
        ...
        if(tA.size() == 2){ // El segundo caso base es si tiene 2 elem, en el cual debo, o swapearlos o no, segun su orden y ya estaria.
            ...
            mergeSort(primeraParte); // Ahora ordeno cada una de las 2 partes
            mergeSort(segundaParte);
            tA = mergeOrdenado( primeraParte, segundaParte); // Y finalmente las junto en una misma estructura. ahora que ambas estan ordenadas.
        }
    }
}
...
void ejercicio1(Red& red){ // La idea de esta funcion es que recibe el tipo red y devuelve la maxima clique, y cual es su influencia total.
    ...
    mergeSort(K); // Los ordeno de manera ascendente.
...
vector<int> amigosDexEnY(int v, vector<int>& K, Red& red){ // Funcion que devuelve todos amigos de una persona entre un vector de personas.
    ...
    for (int i=0; i<K.size(); i++) { // y por cada persona del grupo le pregunto si es amigo de v
        if (red.sonAmigos(v,K[i]))
            res.push_back(i); // si es amigo de el lo agrego a la respuesta.
    }
    return res;
}
